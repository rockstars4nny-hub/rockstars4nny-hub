## 🔍 Live Bug Bounty Findings

### `app.originprotocol.com` – CORS Misconfiguration
...
During a routine reconnaissance session on the Origin Protocol dApp, I identified a Cross‑Origin Resource Sharing (CORS) misconfiguration on the `https://app.originprotocol.com/api` endpoint. The server returned `Access-Control-Allow-Origin: *`, allowing any external website to read the JSON response from authenticated users.
...
| Tool | Purpose |
|------|---------|
| **CICI** (Covert Intelligence Collection Instrument) | Subdomain discovery, HTTP header analysis, and tech stack fingerprinting. |
| **T.A.P.** (The Aetherverse Protocol) | Payload reference and CVE correlation (used to verify attack surface). |
| **Burp Suite** (Community Edition) | Manual request/response inspection, Repeater testing, and PoC capture. |
| **Kali Linux** | Environment for hosting the PoC and running auxiliary enumeration. |
...
1. **Reconnaissance**  
   - Used CICI to map subdomains and identify the live API root (`/api`).  
   - Noted the `Content-Security-Policy` header exposing internal infrastructure (e.g., `api.originprotocol.com`, `origin.squids.live`, `rpc.soniclabs.com`).

2. **Authentication**  
   - Connected my wallet to the dApp to generate authenticated traffic and simulate a real user session.

3. **Header Analysis**  
   - Intercepted the request to `https://app.originprotocol.com/api` using Burp Suite.  
   - Observed the following response headers:  
     - `Access-Control-Allow-Origin: *`  
     - `Access-Control-Allow-Methods: GET, OPTIONS`  
     - `Access-Control-Allow-Headers: Content-Type`

4. **Proof of Concept**  
   - Crafted a simple `fetch()` request to demonstrate that the endpoint can be read cross‑origin.  
   - Provided Burp screenshots showing the response body and the permissive CORS policy.

---

### Impact

- **Data Exfiltration Risk:** An attacker could host a malicious website that, when visited by an authenticated Origin Protocol user, silently reads the `/api` response and exfiltrates wallet‑specific data (if present).  
- **Attack Surface Expansion:** The CSP leak provides attackers with a comprehensive list of internal API endpoints and third‑party services to target.

---

### Remediation Suggested

- Remove `Access-Control-Allow-Origin: *` and replace with a strict whitelist (e.g., `https://app.originprotocol.com`).  
- Review the `Content-Security-Policy` header to avoid exposing internal infrastructure.

---

### Lessons Learned

- **The importance of authenticated testing:** Connecting a wallet (or logging in) is often the difference between a false positive and a valid high‑severity finding.  
- **CORS is everywhere:** Many production apps leave `*` enabled on API roots, thinking it's harmless. It's not.  
- **Tools work best together:** CICI found the surface, Burp proved the exploit, and T.A.P. provided context for the payload landscape.

---

*This finding is part of my ongoing bug bounty journey. More to come as I continue to refine my OSINT and web application testing methodology.*
